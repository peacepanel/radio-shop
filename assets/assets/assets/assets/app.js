(function(){
  ...
  async function loadCategories(){
    try{
      ...
      names.forEach((n,i)=>{ const o=document.createElement('option'); o.value=n; o.textContent=n; sel.appendChild(o); });
  ...
  $('closeImg').onclick = ()=> $('imgModal').hidden=true;
  ...
  function renderCart(){ const el=$('cartItems'); if(!cart.length){ el.textContent='لا توجد عناصر'; return;} el.innerHTML=''; cart.forEach((it,i)=>{ const d=document.createElement('div'); d.className='row sb'; d.style.padding='8px 0'; d.innerHTML=`<div>${it.name}</div><div class="price">${it.price||''}</div>`; el.appendChild(d); }); }
  $('openCart').onclick = ()=>{ $('cart').hidden=false; renderCart(); };
  $('closeCart').onclick = ()=>{ $('cart').hidden=true; };

  // ربط أزرار الراديو
  $('playBtn').onclick = ()=> window.Radio.toggle();
  $('nextBtn').onclick = ()=> window.Radio.next();
  $('vol').oninput = e => window.Radio.volume(e.target.value/100);
