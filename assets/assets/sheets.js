// أدوات قراءة Google Sheets عبر GViz JSON من صفحات GitHub (بدون GAS)
// الصيغ:
// JSON: https://docs.google.com/spreadsheets/d/SS_ID/gviz/tq?tqx=out:json&sheet=SHEET
// CSV : https://docs.google.com/spreadsheets/d/SS_ID/gviz/tq?tqx=out:csv&sheet=SHEET
...
async function getCategoryNamesFromConfigSheet(config) {
  ...
  const { cols, rows } = gvizToRows(j);
  // ابحث عن أول عمود نصي واعتبره قائمة أسماء الأوراق
  const colIdx = 0;
  const names = rows.map(r => String(r[colIdx] || '').trim()).filter(Boolean);
  return names;
}
...
async function getProductsFromSheet(config, sheetName, periodFilter) {
  ...
  // فallback للفهارس إذا مافي رؤوس مطابقة
